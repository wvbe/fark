...
function consoleLogTable(markdown, name, columns, data) {
	if (markdown) {
		console.log('## ' + name);
		console.log();
		console.log(table([columns, ...data], TABLE_MD_OPTIONS));
	} else {
		console.log(name);
...
function concatAllOfAncestors(command, propertyName) {
	return (command.parent ? concatAllOfAncestors(command.parent, propertyName) : []).concat(
...
module.exports = function (command, informerPool, fromMarkdown) {
	...
	consoleLogTable(
		...
		concatAllOfAncestors(command, 'options')
	...
	consoleLogTable(
		...
		informerPool
			.getProps()
			.sort((a, b) => a.name.localeCompare(b.name))
			.map(prop => [prop.name, prop.description ? prop.description : NO_DESCRIPTION])
	);
	...
	consoleLogTable(
		...
		informerPool
			.getFilters()
			.sort((a, b) => a.name.localeCompare(b.name))
			.map(filter => [filter.name, filter.description ? filter.description : NO_DESCRIPTION])
	);
